...
export default function Edit(props) {
	...
	const { positivesTitle, negativesTitle, selectedPost } = attributes;
	...
	return [
		<InspectorControls>
			<PanelBody title="Settings">
				<ReviewsSelect />
				<TextControl
					label="Positives Title"
					value={positivesTitle}
					onChange={(val) => setAttributes({ positivesTitle: val })}
				/>
				<TextControl
					label="Negatives Title"
					value={negativesTitle}
					onChange={(val) => setAttributes({ negativesTitle: val })}
				/>
			</PanelBody>
		</InspectorControls>,
		<div {...useBlockProps()}>
			<ServerSideRender
				block="affreviews/pros-cons"
				attributes={attributes}
				EmptyResponsePlaceholder={() => (
					<Placeholder
						icon={info}
						label={__("Review Positives - Negatives Info", "affreviews")}
					>
						<div
